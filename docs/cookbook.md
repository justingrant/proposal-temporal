...
Similarly, use `Temporal.TimeZone.getOffsetFor()` to do the same thing for the offset as a number of seconds.
...
Also using `Temporal.TimeZone.getOffsetFor()`, we can map a `Temporal.Absolute` instance and two time zones into the signed difference of UTC offsets between those time zones at that instant, as a number of seconds.
