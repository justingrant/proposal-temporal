...
describe('Absolute', () => {
  describe('Structure', () => {
    ...
    describe('Absolute.prototype', () => {
      it('Absolute.prototype.equal is a Function', () => {
        equal(typeof Absolute.prototype.equal, 'function');
      });
    });
  ...
  describe('Absolute.plus works', () => {
    ...
    describe('cross epoch in ms', () => {
      ...
      it(`(${two}).minus({ days: 20, nanoseconds: 1600 }) = ${one}`, () => assert(three.equal(one)));
      it(`(${one}).plus( days: 20, nanoseconds: 1600 }) = ${two}`, () => assert(four.equal(two)));
    });
  ...
  describe('Absolute.equal works', () => {
    const abs1 = Absolute.from('1963-02-13T09:36:29.123456789Z');
    const abs2 = Absolute.from('1976-11-18T15:23:30.123456789Z');
    const abs3 = Absolute.from('1981-12-15T14:34:31.987654321Z');
    it('pre epoch equal', () => assert(abs1.equal(abs1)));
    it('epoch equal', () => assert(abs2.equal(abs2)));
    it('cross epoch unequal', () => assert(!abs1.equal(abs2)));
    it('epoch unequal', () => assert(!abs2.equal(abs3)));
    it("doesn't cast argument", () => {
      throws(() => abs1.equal(abs1.getEpochNanoseconds()), TypeError);
      throws(() => abs1.equal({}), TypeError);
    });
  });
  describe('Absolute.difference works', () => {
    ...
    it(`(${earlier}).plus(${diff}) == (${later})`, () => assert(earlier.plus(diff).equal(later)));
    it(`(${later}).minus(${diff}) == (${earlier})`, () => assert(later.minus(diff).equal(earlier)));
    it("doesn't cast argument", () => {
