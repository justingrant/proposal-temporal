...
describe('DateTime', () => {
  describe('Structure', () => {
    ...
    describe('DateTime.prototype', () => {
      ...
      it('DateTime.prototype.equal is a Function', () => {
        equal(typeof DateTime.prototype.equal, 'function');
      });
  ...
  describe('DateTime.equal() works', () => {
    const dt1 = DateTime.from('1976-11-18T15:23:30.123456789');
    const dt2 = DateTime.from('2019-10-29T10:46:38.271986102');
    it('equal', () => assert(dt1.equal(dt1)));
    it('unequal', () => assert(!dt1.equal(dt2)));
    it("doesn't cast argument", () => {
      throws(() => dt2.equal({ year: 1976, month: 11, day: 18, hour: 15 }), TypeError);
      throws(() => dt2.equal('1976-11-18T15:23:30.123456789'), TypeError);
    });
  });
  describe('date/time maths', () => {
    ...
    ['years', 'months', 'days', 'hours', 'minutes', 'seconds'].forEach((largestUnit) => {
      ...
      it(`(${earlier}).plus(${diff}) == (${later})`, () => earlier.plus(diff).equal(later));
      it(`(${later}).minus(${diff}) == (${earlier})`, () => later.minus(diff).equal(earlier));
    });
  });
  ...
  describe('DateTime.from() works', () => {
    ...
    it('DateTime.from(number) is converted to string', () =>
      assert(DateTime.from(19761118).equal(DateTime.from('19761118'))));
